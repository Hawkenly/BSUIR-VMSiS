...
unsigned int periodInterruption[] = { 0x03 , 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0xA, 0xB, 0xC, 0xD, 0xE, 0xF };
int alarmOn = 0;
char* months[] =
{
    "JANUARY",
...
void setTime();
void MyDelay(unsigned int delayMs, int choose);
void enterTime();
void setAlarm();
void resetAlarm();
    ...
    while (1) {
        ...
        if (kbhit()) {
            ...
            case '2':
                ...
                scanf("%d", &delayMs);
                int choose;
                printf("Choose the frequency:\n1 - 8192\n 2 -4096\n 3 - 2048\n 4 - 1024\n 5 - 512\n 6 - 256\n 7 - 128\n 8 - 64\n 9 - 32\n 10 - 16\n 11 -8\n 12 - 4\n 13 - 2\n");
                scanf("%d", &choose);
                MyDelay(delayMs, choose);
                break;
...
void MyDelay(unsigned int delayMs, int choose)
{
    unsigned int frequency = periodInterruption[choose - 1];
    disable(); // запрет на прерывание
    ...
    outp(0x70, 0xA); // выбор регистра В
    outp(0x71, inp(0x71) | periodInterruption[3]); // 0100 0000
...
void interrupt newTime(...) // новый обработчик прерываний часов
{
    ...
    inp(0x71); // данные по этому адресу (чтение)
