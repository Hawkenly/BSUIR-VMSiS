...
void Menu() {
	...
	while (1) {
		...
		printf("\n3 - Print channels counter value");
		printf("\n0 - Exit");
...
void CounterValue() {
	...
	for (channel = 0; channel < 3; channel++)
	{
		...
		case 0:
		{
			outp(0x43, 0x00); //заносим управляющее слово, 
			//соответствующее команде CLC (запомнить CE) и номеру канала 0
			counterValue = inp(0x40); //чтение младшего байта счетчика канала 0
			counterValue |= inp(0x40) << 8;
			printf("Channel 0x40 word: %04.4X\n", counterValue);
			break;
		}
		case 1:
		{
			outp(0x43, 0x40); //заносим управляющее слово, 
			//соответствующее команде CLC (запомнить CE) и номеру канала 1
			counterValue = inp(0x41); //чтение младшего байта счетчика канала 1
			counterValue |= inp(0x41) << 8;
			printf("Channel 0x41 word: %04.4X\n", counterValue);
			break;
		}
		case 2:
		{
			outp(0x43, 0x80); //заносим управляющее слово, 
			//соответствующее команде CLC (запомнить CE) и номеру канала 2
			counterValue = inp(0x42); //чтение младшего байта счетчика канала 2
			counterValue |= inp(0x42) << 8;
			printf("Channel 0x42 word: %04.4X\n", counterValue);
			break;
		}
		}
	}
