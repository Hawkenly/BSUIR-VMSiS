...
void find_event_default(Events**root);
void find_event_new(Events**root);
void check_id(Events*root,const int *id,int n);
...
void add_part(Events*event,int i){
    ...
    if (choose == 3) {
        if (event->info.short_info.three.brief_remainder == NULL) {
            printf("------------------------\n");
            printf("Input a brief remainder:\n");
            add_field(buf, &event->info.short_info.three.brief_remainder);\
            event->info.short_info.short_info.fsrt=event->info.short_info.three.brief_remainder;



            printf("Input task name:\n");
            add_field(buf, &event->info.short_info.three.task_name);
            event->info.short_info.short_info.scnd=event->info.short_info.three.task_name;
...
void find_event_new(Events**root){
    ...
    if(choose==1){
        ...
        while(temp!=NULL){
            char*ptr=NULL;
            if(temp->info.short_info.one.location==NULL){
                temp=temp->next;
                continue;
            }
            ptr=strstr(temp->info.short_info.one.location,str1);//возвращает указатель на начало совпадения строк
            if(ptr-(temp->info.short_info.one.location)+1==1){
                ptr=strstr(temp->info.short_info.one.time,str2);
                if(ptr-(temp->info.short_info.one.time)+1==1){
                    count++;
                    id[i]=temp->id;
                    i++;
                    output_one(&temp);
    ...
    if(choose==2){
        ...
        while(temp!=NULL){
            char*ptr=NULL;
            if(temp->info.short_info.two.holiday_name==NULL){
                temp=temp->next;
                continue;
            }
            ptr=strstr(temp->info.short_info.two.holiday_name,str1);//возвращает указатель на начало совпадения строк
            if(ptr-(temp->info.short_info.two.holiday_name)+1==1){
                ptr=strstr(temp->info.short_info.two.members,str2);
                if(ptr-(temp->info.short_info.two.members)+1==1){
                    id[i]=temp->id;
                    i++;
                    count++;
                    output_one(&temp);
    ...
    if(choose==3){
        ...
        while(temp!=NULL){
            char*ptr=NULL;
            if(temp->info.short_info.three.brief_remainder==NULL){
                temp=temp->next;
                continue;
            }
            ptr=strstr(temp->info.short_info.three.brief_remainder,str1);//возвращает указатель на начало совпадения строк
            if(ptr-(temp->info.short_info.three.brief_remainder)+1==1){
                ptr=strstr(temp->info.short_info.three.task_name,str2);
                if(ptr-(temp->info.short_info.three.task_name)+1==1){
                    id[i]=temp->id;
                    i++;
                    count++;
                    output_one(&temp);
...
void output_list(Events** root){
    ...
    printf("---------------------------------------------------------------------------------------------------------------------------\n");
    while(temp!=NULL){
        printf("| %d  | %-*s | %s | %-*s |",temp->id, (int)strlen(l_name->name), temp->name,  temp->date, (int)strlen(l_info->info.info),temp->info.info);
